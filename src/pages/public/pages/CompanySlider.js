...
const CompanySlider = () => {
    ...
    return (
        <>
            <Box sx={{ width: '100%', my: spacing(5) }}>
                <Typography variant='h4' sx={{ textTransform: 'uppercase', fontWeight: 'bold', my: 2, color: palette.primary.main }}>Top Companies</Typography>
                        ...
                        <Box component="ul" className="splide__list">
                            {companySlides.map((slide, index) => (
                                <Box component="li" key={index} className="splide__slide">
                                    <img src={slide.image} alt={`Slide ${index + 1}`} style={{ border: '1px solid #dfdfdf', width: '97%' }} />
                                </Box>
                            ))}
                        </Box>
                    </Box>
